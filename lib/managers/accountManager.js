...
module.exports.addCourse = function (userId, courseName) {
  ...
  userModel.findById(userId).then(function (found) {
      ...
      userModel.updateOne(found._id, {c: found.c}).then(function (updatedUser) {
        logger.info('Successfully added course ' + courseName + ' to user with id ' + userId);
        var token = jwt.sign({
          email: updatedUser.em,
